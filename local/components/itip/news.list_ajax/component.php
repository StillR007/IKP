...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	/*if(isset($_GET['type']) && $_GET['type'] != "0") {
		$arFilter['PROPERTY_PROJECT_TYPE'] = $_GET['type'];
	}
	if(isset($_GET['geo']) && $_GET['geo'] != "0") {
		$arFilter['PROPERTY_PROJECT_GEO'] = $_GET['geo'];
	}
	if(isset($_GET['status']) && $_GET['status'] != "0") {
		$arFilter['PROPERTY_PROJECT_STATUS'] = $_GET['status'];
	}
	if(isset($_GET['year']) && $_GET['year'] != "0") {
		$arFilter['PROPERTY_PROJECT_YEAR'] = $_GET['year'];
	}*/
